...
class MyApp extends StatelessWidget {
  ...
  _initJSEngine() async {
    _jsEngine = JSEngine(options: JSEngineOptions(printConsole: true));
    ...
    _jsEngine.createNewFunction(testAddCallback.name, testAddCallback.callbackWrapper);

    /// 2. eval the global script;
    _jsEngine.evalScript(jsCode);

    /// 3. test another batch of script
    await _testCallJS(_jsEngine);

    /// 4. loop the engine to make sure async result are executed
    JSEngine.loop(_jsEngine);
  }

  _testCallJS(JSEngine engine) async {
